...
pacman::p_load(tidyverse,  brms, cmdstanr, kableExtra, here, ape, tidybayes, patchwork, glue, furrr, data.table, dtplyr, tictoc)
...
a_b %>% full_join(a_o, by = c("var", "method", "locomotion", "accuracy")) %>% 
  ...
  group_by(var, method) %>% 
  summarise(across(within1:ordinal, ~ na.omit(.x))) %>% 
  ungroup() %>% 
