...
```{r}
loo(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15, m16)
```
...
```{r}
pre <- epred_draws(m14, newdata = dat %>% filter(!(Genus_species %in% c("Urocyon_cinereoargenteus", "Leopardus_geoffroyi")))) %>% as.data.frame() %>% select(Genus_species, .epred) %>% group_by(Genus_species) %>% summarise(mean = mean(.epred))

dat2 %>% select(Genus_species, Loc_bin) %>% right_join(pre, by = "Genus_species") %>% mutate(rnd = round(mean)) %>% select(!mean) %>% mutate(Resp_final = if_else(Loc_bin == rnd, 1, 0)) %>% 
  view()
 # count(Resp_final)
```
