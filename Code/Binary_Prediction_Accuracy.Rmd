...
accuracy <- preds %>% 
  ...
  mutate(dif = phy_int - no_phy_int) %>% 
  arrange(desc(no_phy_int)) 
...
## Biggest Diffs between Phy and no phy

```{r}
accuracy %>% 
  pivot_wider(names_from  = "method", values_from = "accuracy") %>% 
  mutate(dif = phy_int - no_phy_int) %>% view()
  
```


## 2 Covariate Mult Mod


```{r}
list_m <- read_rds(here("Data","B_lm_mods_multi.Rds"))
```

Testing the predictive accuracy of the two covariate model used in the T. wilsoni predictions. 
...
pred1 <- map(1, ~bin_pred_phy_fn(list_m)) %>% 
  bind_rows() %>% 
  mutate(var = rep(sprintf("m_%d",seq(1:11)), each = 427),
         method = 'phy_int',
         mass = 'log_mass')
...
pred2 <- map(1, ~bin_pred_nophy_fn(list_m)) %>% 
  bind_rows() %>% 
  mutate(var = rep(sprintf("m_%d",seq(1:11)), each = 427),
         method = 'no_phy_int',
         mass = 'log_mass')
...
accs <- accuracy %>%
  #filter(method == "no_phy_int") %>% 
as.vector(accs$var) %>% write_rds(here("Data", "Accuracy_mult_list.rds"))

accuracy %>% write_csv(here("Data", "Accuracy_mult.csv"))
```
