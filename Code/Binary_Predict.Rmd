...
list_b2 <- append(list_b_lm1, list_b_lm2)  %>% keep_at(pred_vars)
...
pred_df <- map2(list_b2, nm, ~nnn(.x, .y)) %>% bind_rows() %>% 
  #write_csv(here("Data", "Fossil_Multi_Predictions.csv")) %>% 
...
Load Models
```{r}
list_b2m <- read_rds(here("Data", "B_lm_mods_multi.Rds"))
```


```{r}
pred_df_ext_full <- list_b2m$mod1 %>% 
    add_epred_draws(newdata = model_dat, 
                    resp = "Locbin",
                    value = "Prob_of_Climbing",
                    #allow_new_levels = TRUE, 
...
df1 <- pred_ext(list_b2m$mod1, fdat %>% filter(Genus_species %in% c('Jeholbaatar_kielanae', 'Plesiadapis_cookei')))

df2 <- pred_ext(list_b2m$mod2, fdat %>% filter(Genus_species %in% c('Eomaia_scansoria', 'Sinodelphys_szalayi')))

df3 <- pred_ext( list_b2m$mod3, fdat %>% filter(Genus_species %in% c('Ambolestes_zhoui')))

df4 <- pred_ext(list_b2m$mod4, fdat %>% filter(Genus_species %in% c('Rugosodon_eurasiaticus')))

df5 <- pred_ext(list_b2m$mod5, fdat %>% filter(Genus_species %in% c('Ignacius_clarkforkensis')))

df6 <- pred_ext(list_b2m$mod6, fdat %>% filter(Genus_species %in% c('Dryomomys_szalayi')))

df7 <- pred_ext(list_b2m$mod7, fdat %>% filter(Genus_species %in% c('Kryptobaatar_dashzevegi')) %>% 
                  select(-Specimen_Num) %>% 
                  mutate(Specimen_Num = "mean_values") %>%
                  group_by(Genus_species, Specimen_Num) %>% 
                  summarise_at(14:93, mean, na.rm = T))

df8 <- pred_ext(list_b2m$mod8, fdat %>% filter(Genus_species %in% c('Juramaia_sinensis', 'Cokotherium_jiufotangensis')))

df9 <- pred_ext(list_b2m$mod9, fdat %>% filter(Genus_species %in% c('Filikomys_primaevus')))

df10 <- pred_ext(list_b2m$mod10, fdat %>% filter(Genus_species %in% c('Ptilodus_kummae')))

df11 <- pred_ext(list_b2m$mod11, fdat %>% filter(Genus_species %in% c('Torrejonia_wilsoni')))
...
pred_all <- read_csv(here("Data", "Binary_Preds.csv")) %>% bind_rows(pred_all2) %>% write_csv(here("Data", "Binary_Preds.csv"))
...
ggsave(here("Plots", "Bin_Pred_Extant2.pdf"), height = 6, width = 13)
ggsave(here("Plots", "Bin_Pred_Extant2.jpg"), height = 6, width = 13)
```
...
ggsave(here("Plots", "Bin_Pred_All.pdf"), height = 16, width = 13)
ggsave(here("Plots", "Bin_Pred_All.jpg"), height = 16, width = 13)
```
