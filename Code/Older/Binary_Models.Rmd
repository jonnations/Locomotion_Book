...
```{r}
dat %>% select(log_Mass:PES2) %>% map(~ mean(is.na(.))) %>% bind_rows() %>% pivot_longer(everything(), names_to = "measure", values_to = "val") %>% arrange(desc(val)) %>% mutate(val = round(val, 2)) %>% write_csv(here("Data", "Missing_Data_Percents.csv"))
```
