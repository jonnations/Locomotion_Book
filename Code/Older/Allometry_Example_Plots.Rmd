...
```{r}
aloplot(log_gm, Mtl)   -> tp1
aloplot(log_gm, lsMtl)   -> tp2

tp1 + tp2 + plot_layout(guides = 'collect')
```
...
```{r}
dat %>% ggplot(aes(y = Loc_bin, x = Ppl, color = log_Mass)) + 
  geom_jitter(width = 0, height = 0.1) +
  scale_color_viridis_c() +
  labs(y = 'Binary Locomotion') +
  theme_bw() -> p1

dat %>% ggplot(aes(y = Loc_bin, x = lsPpl, color = log_Mass)) + 
  geom_jitter(width = 0, height = 0.1) +
  scale_color_viridis_c() +
  labs(y = "") +
  theme_bw() -> p2
