...
Running predictions with multiple predictors based on a dag. This means using **ALL** the top 12 predictors in one model, along with size, and the interaction(s)? 
...
# Fit Models No Interaction

## Log Mass  

Hl,Rl, Ul, Fl, OLI

PES2, MANUS, PES, Ppl, MANUS2, HPI,  Ipl

I think this will be one model with multiple missing variables, then one extra model for each covariate with missing data. 
```{r}

bin_mod <-  bf(Loc_bin ~ mi(PES2) + mi(MANUS) + mi(PES) + mi(Ppl) + mi(MANUS2) + mi(HPI) + mi(Ipl) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()
...
m_form <- bin_mod + PES2_mod + MANUS_mod + PES_mod + Ppl_mod + MANUS2_mod + HPI_mod + Ipl_mod + set_rescor(FALSE)
...
```{r}
bin_mod2 <- bf(Loc_bin ~ mi(PES2) + mi(MANUS) + mi(PES) + mi(Ppl) + mi(MANUS2) + mi(Ipl) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m2_form <- bin_mod2 + PES2_mod + MANUS_mod + PES_mod + Ppl_mod + MANUS2_mod + Ipl_mod + set_rescor(FALSE)
                    
pri = prio(m2_form)

mod2 <- brm(m2_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod2 %>% write_rds(here("Data","B_lm_multi_2.Rds"))
```
...
Dryomomys_szalayi
```{r}
bin_mod3 <- bf(Loc_bin ~ mi(PES2) + mi(MANUS) + mi(PES) + mi(Ppl) + mi(MANUS2) + mi(Ipl) + Hl + Rl + Fl + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m3_form <- bin_mod3 + PES2_mod + MANUS_mod + PES_mod + Ppl_mod + MANUS2_mod +  Ipl_mod + set_rescor(FALSE)
           
pri = prio(m3_form)

mod3 <- brm(m3_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod3 %>% write_rds(here("Data","B_lm_multi_3.Rds"))
```
...
```{r}
bin_mod4 <- bf(Loc_bin ~ mi(PES2) + mi(MANUS) + mi(PES) + mi(Ppl) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m4_form <- bin_mod4 + PES2_mod + MANUS_mod + PES_mod + Ppl_mod  + set_rescor(FALSE)
            
pri = prio(m4_form)

mod4 <- brm(m4_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod4 %>% write_rds(here("Data","B_lm_multi_4.Rds"))
```
...
```{r}
bin_mod5 <- bf(Loc_bin ~ mi(PES2) + mi(PES) + mi(HPI) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m5_form <- bin_mod5 + PES2_mod +  PES_mod + HPI_mod  + set_rescor(FALSE)
           
pri = prio(m5_form)

mod5 <- brm(m5_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod5 %>% write_rds(here("Data","B_lm_multi_5.Rds"))
```
...
```{r}
bin_mod6 <- bf(Loc_bin ~ mi(PES2) + mi(PES) + Fl + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m6_form <- bin_mod6 + PES2_mod  + PES_mod + set_rescor(FALSE)
           
pri = prio(m6_form)

mod6 <- brm(m6_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod6 %>% write_rds(here("Data","B_lm_multi_6.Rds"))
```
...
no pes or pes2
```{r}
bin_mod7 <- bf(Loc_bin ~  mi(MANUS) + mi(HPI) + mi(Ppl) + mi(MANUS2) + mi(Ipl) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m7_form <- bin_mod7  + MANUS_mod + HPI_mod + Ppl_mod + MANUS2_mod +  Ipl_mod + set_rescor(FALSE)
         
pri = prio(m7_form)

mod7 <- brm(m7_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod7 %>% write_rds(here("Data","B_lm_multi_7.Rds"))
```
...
```{r}
bin_mod8 <- bf(Loc_bin ~ mi(MANUS)  + mi(Ppl) + mi(MANUS2) + mi(Ipl) + Hl + Rl + Ul + Fl + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m8_form <- bin_mod8  + MANUS_mod  + Ppl_mod + MANUS2_mod +  Ipl_mod + set_rescor(FALSE)
           
pri = prio(m8_form)

mod8 <- brm(m8_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod8 %>% write_rds(here("Data","B_lm_multi_8.Rds"))
```
...
```{r}
bin_mod9 <- bf(Loc_bin ~ mi(MANUS)  + mi(Ppl) + mi(MANUS2) + mi(Ipl) + Hl + Rl + Ul  + OLI + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m9_form <- bin_mod9  + MANUS_mod  + Ppl_mod + MANUS2_mod +  Ipl_mod + set_rescor(FALSE)
           
pri = prio(m9_form)

mod9 <- brm(m9_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod9 %>% write_rds(here("Data","B_lm_multi_9.Rds"))
```
...
only Ppl haha
```{r}
bin_mod10 <- bf(Loc_bin ~  mi(Ppl) + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m10_form <- bin_mod10 + Ppl_mod + set_rescor(FALSE)
           
pri = prio(m10_form)

mod10 <- brm(m10_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod10 %>% write_rds(here("Data","B_lm_multi_10.Rds"))
```
...
```{r}
bin_mod11 <- bf(Loc_bin ~ mi(HPI) + Hl + Rl + Ul  + OLI + Fl + log_Mass + (1|Genus_species) + (1|gr(Taxon_Upham_style,cov = A))) + bernoulli()

m11_form <- bin_mod11  + HPI_mod + set_rescor(FALSE)
              
pri = prio(m11_form)

mod11 <- brm(m11_form, data=dat, refresh = 0, cores = 2, data2 = list(A = A), prior = pri, seed = 145, control=list(max_treedepth=15)
               )
mod11 %>% write_rds(here("Data","B_lm_multi_11.Rds"))
```
...
df3 <- pred_ext( mod3, fdat %>% filter(Genus_species %in% c('Dryomomys_szalayi')))

df4 <- pred_ext(mod4, fdat %>% filter(Genus_species %in% c('Rugosodon_eurasiaticus')))

df5 <- pred_ext(mod5, fdat %>% filter(Genus_species %in% c('Kryptobaatar_dashzevegi')) %>% 
                  select(-Specimen_Num) %>% 
                  mutate(Specimen_Num = "mean_values") %>%
                  group_by(Genus_species, Specimen_Num) %>% 
                  summarise_at(14:93, mean, na.rm = T))

df6 <- pred_ext(mod6, fdat %>% filter(Genus_species %in% c('Ptilodus_kummae')))

df7 <- pred_ext(mod7, fdat %>% filter(Genus_species %in% c('Ignacius_clarkforkensis')))

df8 <- pred_ext(mod8, fdat %>% filter(Genus_species %in% c('Ambolestes_zhoui')))

df9 <- pred_ext(mod9, fdat %>% filter(Genus_species %in% c('Juramaia_sinensis', 'Cokotherium_jiufotangensis')))

df10 <- pred_ext(mod10, fdat %>% filter(Genus_species %in% c('Torrejonia_wilsoni')))

df11 <- pred_ext(mod11, fdat %>% filter(Genus_species %in% c('Filikomys_primaevus')))

pred_df <- bind_rows(df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11) %>% mutate(var = 'all')
```
...
pred_all <- bind_rows(pred_df_ext_full, pred_df_ext_fos, pred_df) %>% #filter(var %in% var_acc) %>% 
  ...
  write_csv(here("Data", "Bin_Multiple_Predictions.csv"))
