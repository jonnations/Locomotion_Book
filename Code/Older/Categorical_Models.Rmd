...
p1 <- get_prior(Loc_mode_Categorical ~ Sl + log_Mass + (1 | Genus_species) +  (1|gr(Taxon_Upham_style, cov = A)),
  data2 = list(A = A),
           family = categorical(),
           data = dat) %>% 
    mutate(
...
#initial fit
mm <- brm(Loc_mode_Categorical ~ Sl + log_Mass + (1 | Genus_species) +  (1|gr(Taxon_Upham_style, cov = A)),
          data2 = list(A = A),
          family = categorical(),
...
fit_list_c <- vector(mode ="list")

for(i in varis){
  fit_list_c[[i]]<- update(mm,
...
```{r message = FALSE, warning=FALSE}
for(i in fit_list_c){
 plots <- plot(conditional_effects(i, categorical = TRUE), plot=F, points = F)[[1]]
