...
lms <- map(list_o_lm[2:13], cln) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "b_",""))

lms2 <- map(list_o_lm[14:27], cln2) %>% 
...
# Function -> pulls the th column, b_ or the slope, then pivots in long, and scales it to probability
gms <- map(list_o_gm[2:13], cln) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "b_",""))

gms2 <- map(list_o_gm[14:27], cln2) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "bsp_LocOrd2_mi",""))
...
```{r message = FALSE, warning=FALSE}
# Function -> pulls the th column, b_ or the slope, then pivots in long, and scales it to probability
rs <- map(list_o_ratio[2:22], cln) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "b_",""))

rs2 <- map(list_o_ratio[23:44], cln2) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "bsp_LocOrd2_mi",""))
...
ms %>% write_csv(here("Data", "Ordinal_Effects.csv"))
```
...
ggsave(here("Plots", "O_Effects_All.pdf"), height = 8, width = 9)
ggsave(here("Plots", "O_Effects_All.jpg"), height = 8, width = 9)
```
...
ggsave(here("Plots", "O_Effects_Some.pdf"),  height = 3, width = 9)
ggsave(here("Plots", "O_Effects_Some.jpg"),  height = 3, width = 9)
```
...
ggsave(here("Plots", "O_Effects_All_BW.pdf"), height = 8, width = 9)
ggsave(here("Plots", "O_Effects_All_BW.jpg"), height = 8, width = 9)
```
...
ggsave(here("Plots", "O_Effects_Some_BW.pdf"), height = 3, width = 9)
ggsave(here("Plots", "O_Effects_Some_BW.jpg"), height = 3, width = 9)
```
### Plot Accurate v2
...
ggsave(here("Plots", "O_Effects_Somev2_BW.pdf"), height = 11, width = 8)
ggsave(here("Plots", "O_Effects_Somev2_BW.jpg"), height = 11, width = 8)
```
...
lms2 <- map(list_o_lm[14:27], cln2) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "bsp_LocOrd2_mi",""), mass = "lm")
...
gms2 <- map(list_o_gm[14:27], cln2) %>% 
  bind_rows() %>% mutate(var = str_replace(var, "bsp_LocOrd2_mi",""), mass = "gm")
...
ggsave(here("Plots", "O_Mass_All.jpg"), height = 8, width = 9)
```
