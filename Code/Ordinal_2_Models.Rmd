...
Predicting Ordinal Locomotion classifications with 4 rankings using postcranial measurements. Models will also incorporate intraspecific variation and phylogenetic variation.
...
# Recode Ordinal Rankings
  mutate(Loc_Ord = case_when(Loc_mode_Ordinal == "G"  ~ "G",
         ...
         Loc_Ord2 = fct_relevel(Loc_Ord2, c("A","Sc","T","B")),
...
```{r}
pal = c('#62361B',  '#e6ab02', '#66a61e', '#7570b3')
...
```{r}
p1 <- plot(conditional_effects(list_o2_lm$OLI, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal)+ ylim(0,1) + xlim(NA, 4)

p2 <- plot(conditional_effects(list_o2_lm$MANUS, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)

p3 <- plot(conditional_effects(list_o2_lm$Uol, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)

p4 <- plot(conditional_effects(list_o2_lm$Ppl, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)

(p1 + p2) / (p3 + p4) + plot_layout(guides = "collect") + plot_annotation(title = "log mass L column, Geo Mean R column") & theme_bw()

ggsave( here("Plots","Ord2_compare_1.pdf"))
```

```{r}
p1g <- plot(conditional_effects(list_o2_gm$OLI, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal)+ ylim(0,1) + xlim(NA, 4)

p2g <- plot(conditional_effects(list_o2_gm$MANUS, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)

p3g <- plot(conditional_effects(list_o2_gm$Uol, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)

p4g <- plot(conditional_effects(list_o2_gm$Ppl, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) + ylim(0,1)
...
ggsave( here("Plots","Ord2_compare_2.pdf"), height = 10)
```


```{r}
plot(conditional_effects(list_o2_lm$Hl, categorical = TRUE, prob = 0.89), plot=F, points = T)[[1]] +
  scale_fill_manual(values = pal) +
...
ms_01.3 <- brm( Loc_Ord2 ~ Uol + log_Mass +  (1|Genus_species)+ (1|gr(Taxon_Upham_style,cov = A)), family = cumulative(), data=dat, refresh = 0, cores = 4, data2 = list(A = A), prior = ps1, control = list(adapt_delta = .99))


ms_01.4 <- brm( Loc_Ord2 ~ Uol + log_Mass +  (1|gr(Taxon_Upham_style,cov = A)), family = cumulative(), data=dat, refresh = 0, cores = 4, control = list(adapt_delta = .99),
               data2 = list(A = A),
               prior = c(prior(student_t(3,0,2), class = Intercept),
                prior(normal(0, 1), class = b),
                prior(normal(0, 1), class = sd)))

ms_01.5 <- brm( Loc_Ord2 ~ Uol + log_Mass , family = cumulative(), data=dat, refresh = 0, cores = 4, prior = c(prior(student_t(3,0,2), class = Intercept),
                prior(normal(0, 1), class = b)))
