...
extant_ground <- c("Solenodon_paradoxus", "Atelerix_albiventris", "Sorex_trowbridgii", "Gerbillus_gerbillus", "Monodelphis_domestica")
...
pred_all <- bind_rows(pred_df_ext_full, pred_df_ext_fos, pred_df) %>% filter(var %in% var_acc) %>% 
  ...
  write_csv(here("Data", "All_Int_Predictions.csv"))
```
...
ggsave(here("Plots", "Predict_Int_Fossil.pdf"), height = 8, width = 10)
...
ggsave(here("Plots", "Predict_Int_Extant.pdf"), height = 10, width = 10)
...
ggsave(here("Plots", "Predict_Int_All.pdf"), height = 16, width = 10)
```
