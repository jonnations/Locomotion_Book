...
```{r}
tt %>% filter(var != "log_Mass") %>% ggplot(aes(x = effect1__, y = estimate__)) + 
...
tt %>%ggplot(aes(x = effect1__, y = estimate__)) + 
  geom_vline(xintercept = 0, linetype = 2, linewidth = 0.25) +
...
```{r}
tt %>% filter(var != "gm") %>% ggplot(aes(x = effect1__, y = estimate__)) + 
  geom_vline(xintercept = 0, linetype = 2, linewidth = 0.25) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.2)+
  geom_line(linewidth = 1, position = position_dodge(0.05)) + 
  scale_fill_manual(values = pal) +
